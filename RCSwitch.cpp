...
#ifdef PIGPIO_H
  gpioSetMode( this->nTransmitterPin, PI_INPUT);
#endif
...
#ifdef PIGPIO_H
    gpioSetAlertFunc(this->nReceiverInterrupt, &handleInterrupt);
#else
...
#ifdef PIGPIO_H
void RCSwitch::handleInterrupt(int gpio, int level, uint32_t tick) {
#else
...
// disk           this->disableReceive();
       ...
       gpioSleep(PI_TIME_RELATIVE, 0, nHighPulses); 
       gpioWrite(this->nTransmitterPin, PI_LOW);
       gpioSleep(PI_TIME_RELATIVE, 0, nLowPulses); 
#else
